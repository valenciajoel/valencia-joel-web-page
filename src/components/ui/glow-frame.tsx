...
export function GlowFrame({ children, className, ...props }: GlowFrameProps) {
  return (
    <div
      className={cn(
        'group relative rounded-xl border border-border-active/20 bg-secondary/50 p-6 transition-colors duration-500 hover:border-primary/50',
        className
      )}
      {...props}
    >
      {/* Inner shimmer wrapper — overflow-hidden here keeps shimmer inside the card */}
